...
const handleShareConversation = () => {
  // Retrieve askMistralMessages from localStorage
  const askMistralMessages = localStorage.getItem('askMistralMessages');

  if (!askMistralMessages) {
  ...
  try {
    ...
    const messages = JSON.parse(askMistralMessages);

    // Build a readable text representation of the conversation
    let conversationText = "";
    messages.forEach((message, index) => {
      if (message.fromUser) {
        conversationText += `Moi: ${message.question}\n\n`;
      } else {
        conversationText += `JobBot: ${message.answer}\n\n`;
      }
    });

    // Set the retrieved message into the message input field
    setnewMessage({
      ...newMessage,
      message: conversationText.trim(), // Trim to remove trailing newline if needed
    });

    // Optionally, you can trigger the SendMessage function here if needed
    //SendMessage();
  } catch (error) {
    console.error("Error parsing or processing messages:", error);
              ...
              <div className="col-12 col-lg-5 col-xl-3 border-right">
                ...
                {messages.map((message) => (
                  <Link 
                    ...
                    key={message.id} // Ajouter une clé unique pour chaque message
                  >
                    <small>
                    ...
                    <div className="d-flex align-items-start">
                      ...
                      <div className="flex-grow-1 ml-3">
                        ...
                        <div className="small">
                          <small>
                            {message.message.length > 50 ? // Limiter à 50 caractères
                              message.message.substring(0, 50) + '...' // Afficher les premiers 50 caractères suivis de "..."
                              : message.message // Sinon, afficher le message complet
                            }
                          </small>
                        </div>
                      </div>
              ...
              <div className="col-12 col-lg-7 col-xl-9">
                ...
                <div className="flex-grow-0 py-3 px-4 border-top">
                  <div className="input-group">
                    ...
                    <button onClick={handleShareConversation} className="btn btn-secondary">Partager la conversation JobBot</button>
                  </div>
                </div>
              </div>
            </div>
